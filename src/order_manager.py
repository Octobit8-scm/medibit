...
class OrderManager:
    ...
    def generate_pdf_order(self, order_items, order_id, timestamp, supplier_info=None):
        ...
        pharmacy_name = pharmacy_details.name if pharmacy_details else "medibit Pharmacy"
        ...
        if pharmacy_details:
            ...
            if pharmacy_details.license_number:
                pharmacy_data.append(['License Number:', pharmacy_details.license_number])
        ...
        # Items Table
        items_data = [['S.No', 'Medicine Name', 'Barcode', 'Current Stock', 'Order Quantity', 'Manufacturer']]
        ...
        items_table = Table(items_data, colWidths=[0.5*inch, 2*inch, 1.5*inch, 1*inch, 1*inch, 1.5*inch])
        ...
        summary_text = f"Total Items: {len(order_items)} | Total Quantity: {total_quantity}"
    ...
    def send_order_email(self, supplier_info, pdf_path, order_items, order_id):
        ...
        try:
            ...
            msg['Subject'] = f"Purchase Order #{order_id} - medibit Pharmacy - {datetime.now().strftime('%Y-%m-%d')}"
...
medibit Pharmacy Team
            ...
            response = requests.post(url, data=text_payload, auth=(account_sid, auth_token))
            
            if response.status_code in [200, 201]:
                return True, f"Order notification sent to {supplier_info['phone']} (detailed text + PDF via email)"
            else:
    ...
    def send_order_to_supplier(self, supplier_info, order_items, order_id, timestamp):
        ...
        # Generate PDF order
        try:
            pdf_path = self.generate_pdf_order(order_items, order_id, timestamp, supplier_info)
            results.append(("PDF Generation", True, f"Order saved to: {pdf_path}"))
        except Exception as e:
            results.append(("PDF Generation", False, f"Failed to generate PDF: {str(e)}"))
            return results
        
        # Send via email
        if self.config["email"]["enabled"] and supplier_info.get('email'):
            success, message = self.send_order_email(supplier_info, pdf_path, order_items, order_id)
            results.append(("Email", success, message))
        
        # Send via WhatsApp
        if self.config["whatsapp"]["enabled"] and supplier_info.get('phone'):
            success, message = self.send_order_whatsapp(supplier_info, pdf_path, order_items, order_id)
