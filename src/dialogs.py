...
class AddMedicineDialog(QDialog):
    def __init__(self, parent=None, barcode=None):
        ...
        # Add helpful note
        note_label = QLabel("Note: If a medicine with this barcode already exists, the quantity will be added to the existing stock.")
...
class NotificationSettingsDialog(QDialog):
    ...
    def init_ui(self):
        ...
        self.email_enabled.setChecked(self.notification_manager.config["email"]["enabled"])
        email_layout.addWidget(self.email_enabled)
        
        # Email form
        email_form = QFormLayout()
        self.smtp_server = QLineEdit(self.notification_manager.config["email"]["smtp_server"])
        self.smtp_port = QSpinBox()
        self.smtp_port.setRange(1, 65535)
        self.smtp_port.setValue(self.notification_manager.config["email"]["smtp_port"])
        self.sender_email = QLineEdit(self.notification_manager.config["email"]["sender_email"])
        self.sender_password = QLineEdit(self.notification_manager.config["email"]["sender_password"])
        self.sender_password.setEchoMode(QLineEdit.Password)
        self.sender_password.setPlaceholderText("Use App Password for Gmail (not regular password)")
        self.recipient_emails = QLineEdit(", ".join(self.notification_manager.config["email"]["recipient_emails"]))
        self.recipient_emails.setPlaceholderText("email1@example.com, email2@example.com")
        
        # Add helpful note about Gmail App Passwords
        gmail_note = QLabel("Note: For Gmail, you need to use an App Password instead of your regular password.\nGo to Google Account → Security → 2-Step Verification → App passwords")
        ...
        whatsapp_title.setStyleSheet("font-size: 16px; font-weight: bold; color: #25d366;")
        ...
        self.whatsapp_enabled.setChecked(self.notification_manager.config["whatsapp"]["enabled"])
        whatsapp_layout.addWidget(self.whatsapp_enabled)
        
        # WhatsApp form
        whatsapp_form = QFormLayout()
        self.whatsapp_api_key = QLineEdit(self.notification_manager.config["whatsapp"]["api_key"])
        self.whatsapp_api_key.setPlaceholderText("Account SID:Auth Token (for Twilio)")
        self.whatsapp_phone_numbers = QLineEdit(", ".join(self.notification_manager.config["whatsapp"]["phone_numbers"]))
        ...
        self.sms_phone_numbers = QLineEdit(", ".join(self.notification_manager.config["sms"]["phone_numbers"]))
    ...
    def save_settings(self):
        ...
        try:
            ...
            recipient_emails = [email.strip() for email in self.recipient_emails.text().split(",") if email.strip()]
            self.notification_manager.update_config("email", "recipient_emails", recipient_emails)
            ...
            whatsapp_phones = [phone.strip() for phone in self.whatsapp_phone_numbers.text().split(",") if phone.strip()]
            self.notification_manager.update_config("whatsapp", "phone_numbers", whatsapp_phones)
            ...
            sms_phones = [phone.strip() for phone in self.sms_phone_numbers.text().split(",") if phone.strip()]
...
class ThresholdSettingDialog(QDialog):
    ...
    def init_ui(self):
        ...
        # Add helpful note
        note_label = QLabel("When stock falls below this threshold, you'll receive low stock alerts.")
        note_label.setStyleSheet("color: #666; font-size: 11px; font-style: italic;")
        note_label.setWordWrap(True)
    ...
    def save_threshold(self):
        try:
            ...
            success, error = update_medicine_threshold(self.medicine.barcode, new_threshold)
            
            if success:
                QMessageBox.information(self, "Success", f"Threshold updated to {new_threshold} units for {self.medicine.name}")
                self.accept()
            else:
                QMessageBox.warning(self, "Error", f"Failed to update threshold: {error}")
...
class BulkThresholdDialog(QDialog):
    ...
    def init_ui(self):
        ...
        # Instructions
        instructions = QLabel("Set individual stock thresholds for each medicine. Double-click on a threshold value to edit it.")
        ...
        self.table.setHorizontalHeaderLabels(["Medicine", "Current Stock", "Current Threshold", "New Threshold"])
    ...
    def save_all_thresholds(self):
        ...
        try:
            ...
            for i, med in enumerate(self.medicines):
                ...
                if new_threshold_item:
                    try:
                        ...
                        success, error = update_medicine_threshold(med.barcode, new_threshold)
                        if success:
                            updated_count += 1
                        else:
            ...
            if updated_count > 0:
                QMessageBox.information(self, "Success", f"Updated thresholds for {updated_count} medicines.")
                self.accept()
            elif not errors:
                QMessageBox.information(self, "No Changes", "No thresholds were changed.")
...
class EditMedicineDialog(QDialog):
    ...
    def init_ui(self):
        ...
        if self.medicine.expiry:
            self.expiry.setDate(QDate.fromString(str(self.medicine.expiry), "yyyy-MM-dd"))
        else:
            self.expiry.setDate(QDate.currentDate())
    ...
    def save_changes(self):
        try:
            ...
            if success:
                QMessageBox.information(self, "Success", f"Medicine '{updated_data['name']}' updated successfully!")
                self.accept()
            else:
                QMessageBox.warning(self, "Error", f"Failed to update medicine: {error}")
...
class PharmacyDetailsDialog(QDialog):
    ...
    def load_existing_details(self):
        ...
        if details:
            ...
            print(f"Loaded pharmacy details: {details.name}, {details.address}, {details.phone}, {details.email}")
        else:
...
class QuickAddStockDialog(QDialog):
    ...
    def init_ui(self):
        ...
        # Instructions
        instructions = QLabel("Quickly add stock to existing medicines. Enter quantities to add for each medicine.")
        instructions.setStyleSheet("color: #666; font-size: 12px; font-style: italic;")
        instructions.setWordWrap(True)
        layout.addWidget(instructions)
        ...
        self.table = QTableWidget()
        self.table.setColumnCount(4)
        self.table.setHorizontalHeaderLabels(["Medicine", "Current Stock", "Add Quantity", "New Total"])
        self.table.setRowCount(len(self.medicines))
        self.table.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
        self.table.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
    ...
    def add_stock(self):
        ...
        try:
                ...
                if add_qty_item:
                    try:
                        ...
                        if add_qty > 0:  # Only update if adding stock
                            ...
                            success, error = update_medicine_quantity(med.barcode, new_total)
            ...
            if updated_count > 0:
                QMessageBox.information(self, "Success", f"Added stock to {updated_count} medicines.")
                self.accept()
            else:
                QMessageBox.information(self, "No Changes", "No stock was added. Enter quantities greater than 0 to add stock.")
...
class BillingAddMedicineDialog(QDialog):
    ...
    def filter_table(self, text):
        filtered = [m for m in self.medicines if text.lower() in m.name.lower() or text.lower() in m.barcode.lower()]
