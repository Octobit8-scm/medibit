...
class NotificationManager:
    ...
    def send_email_alert(self, low_stock_medicines):
        ...
        try:
            ...
            msg['Subject'] = f"Low Stock Alert - {datetime.now().strftime('%Y-%m-%d %H:%M')}"
            ...
            server.quit()
            return True, f"Email alert sent to {len(self.config['email']['recipient_emails'])} recipients"
            
        except smtplib.SMTPAuthenticationError as e:
            if "534" in str(e) and "application specific password" in str(e).lower():
                return False, f"Email alert failed: Gmail requires an App Password. Please generate one at: Google Account → Security → 2-Step Verification → App passwords"
            else:
    ...
    def send_whatsapp_alert(self, low_stock_medicines):
        ...
        try:
            ...
            if ":" not in api_key:
                return False, "Invalid Twilio API key format. Use: Account SID:Auth Token"
    ...
    def send_sms_alert(self, low_stock_medicines):
        ...
        try:
            ...
            for i, med in enumerate(low_stock_medicines[:3]):  # Limit to first 3 medicines
            ...
            for phone in self.config["sms"]["phone_numbers"]:
                try:
                    ...
                    from_number = "+16203178530"  # Replace with your Twilio phone number
                    ...
                    if response.status_code in [200, 201]:
                        ...
                        if "21408" in error_msg:
                            print(f"  → Solution: Verify {phone} in Twilio Console → Phone Numbers → Verified Caller IDs")
                        elif "21211" in error_msg:
                            print(f"  → Solution: Check phone number format for {phone}")
                        elif "21214" in error_msg:
                            print(f"  → Solution: Check Twilio phone number in notifications.py")
    ...
    def send_daily_sales_summary_email(self, sales_summary, bill_details):
        ...
        try:
            ...
            msg['From'] = self.config["email"]["sender_email"]
            msg['To'] = ", ".join(self.config["email"]["recipient_emails"])
            msg['Subject'] = f"Daily Sales Summary - {datetime.now().strftime('%Y-%m-%d')}"
            ...
            for recipient in self.config["email"]["recipient_emails"]:
                server.send_message(msg)
            server.quit()
            return True, f"Daily sales summary sent to {len(self.config['email']['recipient_emails'])} recipients"
        except Exception as e:
    ...
    def send_daily_sales_summary_whatsapp(self, sales_summary, bill_details):
        ...
        try:
            ...
            if ":" not in api_key:
                return False, "Invalid Twilio API key format. Use: Account SID:Auth Token"
            account_sid, auth_token = api_key.split(":", 1)
            message = f"📊 *Daily Sales Summary - {datetime.now().strftime('%Y-%m-%d')}*\n\n"
            ...
            if success_count > 0:
                return True, f"WhatsApp daily sales summary sent to {success_count} recipients"
            else:
                return False, "WhatsApp daily sales summary failed for all recipients"
        except Exception as e:
