...
class ReceiptManager:
    ...
    def generate_pdf_receipt(self, customer_info, items, total, timestamp, receipt_id):
        ...
        # Items Table
        items_data = [['S.No', 'Medicine Name', 'Barcode', 'Price (₹)', 'Qty', 'Subtotal (₹)']]
        ...
        for i, item in enumerate(items, 1):
            try:
                ...
                items_data.append([
                    str(i), 'ERROR', 'ERROR', '0.00', '0', '0.00'
                ])
        
        items_table = Table(items_data, colWidths=[0.5*inch, 2*inch, 1.5*inch, 1*inch, 0.5*inch, 1.5*inch])
    ...
    def send_receipt_email(self, customer_info, pdf_path):
        ...
        try:
            ...
            msg['Subject'] = f"Your medibit Pharmacy Receipt - {datetime.now().strftime('%Y-%m-%d')}"
...
medibit Pharmacy Team
            ...
            if response.status_code in [200, 201]:
                return True, f"WhatsApp receipt sent to {customer_info['phone']} (detailed text + PDF via email)"
            else:
    ...
    def send_receipt_to_customer(self, customer_info, items, total, timestamp, receipt_id):
        """Send receipt to customer via all enabled channels"""
        results = []
        
        # Generate PDF receipt
        try:
            pdf_path = self.generate_pdf_receipt(customer_info, items, total, timestamp, receipt_id)
